...
void add_bus() {
    ...
    while (check_bus_number_exists(new_bus.bus_number));

    printf("请输入司机姓名：");
    scanf("%s", new_bus.driver_name);

    printf("请输入司机工号：");
    scanf("%s", new_bus.driver_number);
    ...
    char data[255];
    sprintf(data, "班车编号: %s, 司机姓名: %s, 司机工号: %s, 出发时间: %s, 到达时间: %s, 出发地: %s, 目的地: %s",
            new_bus.bus_number, new_bus.driver_name, new_bus.driver_number, new_bus.departure_time, new_bus.arrival_time, new_bus.from, new_bus.to);
